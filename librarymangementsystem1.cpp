...
//	student st1;
	...
	while(fp.read((char *)&st,sizeof(student)) && found==0)
	{
		...
		if(my_strcmpi(st.retadmno(),sno)==0)
		{
			found=1;
			if(st.rettoken()==0)
			{
	...
	while(fp.read((char *)&st,sizeof(student)) && found==0);
	{
		...
		if(my_strcmpi(st.retadmno(),sno)==0)
		{
			...
			if(st.rettoken()==1)
			{
				...
				{
					if(my_strcmpi(bk.retbno(),st.retstbno())==0)
					{
						bk.display_book();
						flag=1;
